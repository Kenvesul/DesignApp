 ...
 *
 * Project Dashboard — Sprint 16.
 * Shows all completed analyses in the current session.
 * One-click unified project PDF export via /api/project/export/pdf.
 *
 * Refs: EC7 §2.1 — Basis of design: combined calculation set
 */
...
const ANALYSIS_ROUTES = [
  { key: "slope",       label: "⛰ Slope Stability",     to: "/slope",       type: "slope"      },
  { key: "foundation",  label: "🏛 Foundation Bearing",  to: "/foundation",  type: "foundation" },
  { key: "wall",        label: "🧱 Retaining Wall",      to: "/wall",        type: "wall"       },
  { key: "pile",        label: "📌 Pile Capacity",       to: "/pile",        type: "pile"       },
  { key: "sheet_pile",  label: "🗂 Sheet Pile",          to: "/sheet-pile",  type: "sheet-pile" },
];
...
export default function ProjectDashboard() {
  ...
  // Poll /api/health to discover which analyses are in session
  useEffect(() => {
    fetch("/api/health")
      .then(r => r.json())
  ...
  const completedCount = session
    ? Object.values(session).filter(Boolean).length
    : 0;
    ...
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Combine completed analyses into one stamped EC7 calculation set
        </p>
      </div>
      ...
      {!loading && (
        <>
          ...
          <div className="card space-y-3">
            ...
            <div className="text-xs text-gray-400 dark:text-gray-500 space-y-1 border-t dark:border-gray-700 pt-3">
              ...
              <ul className="list-disc list-inside space-y-0.5 ml-2">
                ...
                <li>DA1 combination tables with PASS/FAIL indicators</li>
                <li>Plots: slope cross-section, foundation Boussinesq isobars, wall pressure diagram</li>
              </ul>
            </div>
          </div>
          ...
          {/* Quick links */}
          {completedCount === 0 && (
            <div className="card border-dashed border-gray-300 dark:border-gray-600 text-center py-8">
              <p className="text-gray-400 dark:text-gray-500 mb-4 text-sm">
                Start an analysis to populate the dashboard
              </p>
              <div className="flex flex-wrap justify-center gap-3">
                {ANALYSIS_ROUTES.slice(0, 3).map(({ to, label }) => (
                  <Link key={to} to={to} className="btn-secondary text-sm">
                    {label}
                  </Link>
                ))}
              </div>
