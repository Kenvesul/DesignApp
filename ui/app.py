...
def _slim_for_session(result: dict) -> dict:
    """
    Strip large regenerable keys before storing a result in the Flask session.

    BUG-1 fix: Flask's cookie-based session has a 4 KB limit. Full slope
    results include 'slices' and 'search_cache' which push combined sessions
    over the limit, silently truncating the cookie and breaking export routes.

    The exporters rebuild these internally from 'critical_circle', so stripping
    them here is safe — no EC7 numeric output is lost.

    Keys stripped:
      - 'slices'       — per-slice force tables (~1–2 KB each)
      - 'search_cache' — grid search intermediate results (~0.5–1 KB)
      - 'stem.diagram' — wall stem diagram (regenerated by wall exporter)
    """
    drop = {"slices", "search_cache"}
    slim = {k: v for k, v in result.items() if k not in drop}
    if "stem" in slim and isinstance(slim["stem"], dict):
        slim["stem"] = {k: v for k, v in slim["stem"].items() if k != "diagram"}
    return slim


def _send_temp_file(path: str, download_name: str, mimetype: str):
    """
    Stream a temporary file to the client then delete it after the response.
...
def foundation_analyse():
    ...
    if not result.get("ok"):
        return render_template("foundation_form.html",
                               ...
                               prev=request.form)
    session["last_foundation"]   = _slim_for_session(result)
...
def foundation_export_pdf():
    ...
    return _send_temp_file(path, "foundation_bearing.pdf", "application/pdf")
...
def foundation_export_docx():
    ...
    return _send_temp_file(path, "foundation_bearing.docx",
...
def wall_analyse():
    ...
    if not result.get("ok"):
        return render_template("wall_form.html",
                               ...
                               errors=[result.get("error", "Unknown error")],
                               prev=request.form)
    session["last_wall"]       = _slim_for_session(result)
...
def wall_export_pdf():
    ...
    return _send_temp_file(path, "retaining_wall.pdf", "application/pdf")
...
def wall_export_docx():
    ...
    return _send_temp_file(path, "retaining_wall.docx",
                           "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
...
def sheet_pile_analyse():
    ...
    session["last_sheet_pile"]   = _slim_for_session(result)  # ← B-14 key fix
...
def project_export_pdf():
    ...
    return _send_temp_file(path, "project_calculations.pdf", "application/pdf")
...
def api_sheet_pile_analyse():
    ...
    result = api.run_sheet_pile_analysis(params)
    if result.get("ok"):
        session["last_sheet_pile"]  = _slim_for_session(result)
        session["sp_project"]       = params.get("project", "DesignApp")
        session["sp_job_ref"]       = params.get("job_ref", "")
        session["sp_calc_by"]       = params.get("calc_by", "")
        session["sp_checked_by"]    = params.get("checked_by", "")
